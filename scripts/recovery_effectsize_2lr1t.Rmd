...
if(newparams==1){
  ...
  winlr<-generate_bounded_data(n_con=sum(paper_params$group==0),n_pat=sum(paper_params$group==1),effsize=effectsize,0,1,tol)
  losslr<-generate_bounded_data(n_con=sum(paper_params$group==0),n_pat=sum(paper_params$group==1),effsize=0,0,1,tol)
  
  paper_params$win_alpha[paper_params$group==1]<-winlr$pat
  paper_params$win_alpha[paper_params$group==0]<-winlr$con
  paper_params$loss_alpha[paper_params$group==1]<-losslr$pat
  paper_params$loss_alpha[paper_params$group==0]<-losslr$con
  ...
  paper_params<-paper_params%>%
    select(study,group,win_alpha,loss_alpha,beta)
...
if(newchoices==1){
  ...
  for (taskname in tasklist){
    ...
    for (participant in 1:length(paper_params$group)){
      parameters<-list(alphawin=paper_params$win_alpha[participant],alphaloss=paper_params$loss_alpha[participant],beta=paper_params$beta[participant])  
...
cohen.d(paper_params$win_alpha[paper_params$group==1],paper_params$win_alpha[paper_params$group==0])
cohen.d(parameters_wide$alpha_win[parameters_wide$group=='patients'],parameters_wide$alpha_win[parameters_wide$group=='controls'],na.rm=TRUE)
cohen.d(get_posterior_mean(fit_2lr1t_PA,'alpha_win'),get_posterior_mean(fit_2lr1t_HC,'alpha_win'))

cohen.d(paper_params$loss_alpha[paper_params$group==1],paper_params$loss_alpha[paper_params$group==0])
cohen.d(parameters_wide$alpha_loss[parameters_wide$group=='patients'],parameters_wide$alpha_loss[parameters_wide$group=='controls'],na.rm=TRUE)
cohen.d(get_posterior_mean(fit_2lr1t_PA,'alpha_loss'),get_posterior_mean(fit_2lr1t_HC,'alpha_loss'))
...
cohen.d(get_posterior_mean(fit_2lr1t_PA,'beta'),get_posterior_mean(fit_2lr1t_HC,'beta'))
