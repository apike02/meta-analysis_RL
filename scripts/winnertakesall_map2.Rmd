...
ggsave(paste0(workingdir,'/figures/params_overall_map_2p.png'), scale=0.6, width=18, height =8)
...
for (t in 1:length(tasklist)){
  task<-tasklist[t]
  overall[[t]]<-generate_choices_fromfit(model,task,workingdir,nprior=2,method='map')
...
accuracyplot<-ggplot(accuracy_df,aes(x=trial,y=value,colour=type))+
  ...
  ylim(c(0,1))
ggsave(filename=paste0(workingdir,'/figures/map2_replication_accuracy.png'),accuracyplot,width=10,height=6)

densityplot<-ggplot(matching_df,aes(x=replicated,group=task,colour=task))+
  geom_density()+
  theme_classic()
ggsave(filename=paste0(workingdir,'/figures/map2_replication.png'))
...
ggsave(filename=paste0(workingdir,'/figures/map2_replication_rewards.png'))

accuracyplot + scale_colour_discrete(labels=c('Bank data','Synthetic choices \n from 1lr1t1d1p')) + correlationplot + plot_annotation(tag_levels = 'A')
ggsave(filename=paste0(workingdir,'/figures/map2_choice_replication_plot.png'),width=12,height=4)
```
