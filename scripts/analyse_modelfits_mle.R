analyse_modelfits_mle<-function(model_details,task,ntrials) {
  ...
  matlab_data<-readMat(paste0('N:/Alex/metaRL/mle/results_mle_',task,'.mat'))
  ...
  save(ll_df,file=paste0('N:/Alex/metaRL/big_outputs/ll_df_mle_',task))
  ...
  save(bic_df,file=paste0('N:/Alex/metaRL/big_outputs/bic_df_mle_',task))
  ...
  ggsave(paste0('N:/Alex/metaRL/mle/figures/bic_sums_mle_',task,'.png'),bic_sums,scale=0.4,width=10,height=10)
  ...
  bic_sums_unordered<-ggplot(bic_df_long,aes(x=variable,y=value,fill=variable))+
    ...
    coord_cartesian(ylim = c(round_any(min(colSums(bic_df)),1000,floor), 
                             round_any(max(colSums(bic_df)),1000,ceiling)))
  ggsave(paste0('N:/Alex/metaRL/mle/figures/bic_sums_mle_unordered_',task,'.png'),bic_sums_unordered,scale=0.5,width=10,height=10)
  ...
  ggplot(bf.bic,aes(x=Model,y=log(BayesFactor),fill=Model))+
    geom_bar(stat='identity')
  ggsave(paste0('N:/Alex/metaRL/mle/figures/bf_bic_mle_',task,'.png'),scale=0.5,width=10,height=10)
