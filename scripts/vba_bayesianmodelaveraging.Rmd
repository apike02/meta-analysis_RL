...
for (analysis in 1:length(multiverse_list)){
    ...
    for (i in 1:length(mnames)) {
      ...
      if (multiverse_list[analysis]=='vba_2'){
        ...
        if (cluster == TRUE) {
          load(paste(
            workingdir,
            'generate_recover/effectsize_0.5/',
            loadname_pat,
            '.RData',
            sep = ''
          ))
        ...
        if (multiverse_list[analysis]=='map_1'){
          matlab_data<-readMat(paste0(workingdir,'generate_recover/effectsize_0.5/results_map_',task,'_sp.mat'))
        } else if (multiverse_list[analysis]=='map_2'){
          matlab_data<-readMat(paste0(workingdir,'generate_recover/effectsize_0.5/results_map_',task,'.mat'))
        } else if (multiverse_list[analysis]=='mle'){
          matlab_data<-readMat(paste0(workingdir,'generate_recover/effectsize_0.5/results_mle_',task,'.mat'))
        }
  ...
  parameters_wide<-data.frame(bind_rows(parameters_wide, .id = 'task'))%>%
   ...
   save(parameters_wide,file=paste0(workingdir,'generate_recover/effectsize_0.5/parameters_bma_',multiverse_list[analysis])) #saves before imputing 1
...
Illustrative plot for effect of task on correlations
```{r}

correlations<-list()

load(paste0(workingdir,'generate_recover/effectsize_0.5/paper_params'))

for (analysis in 1:length(multiverse_list)){
  load(file=paste0(workingdir,'generate_recover/effectsize_0.5/parameters_bma_',multiverse_list[analysis]))
  correlations[[analysis]]<-parameters_wide%>%
    filter(draw=='X1')%>%
    mutate(group=factor(group,ordered=TRUE,levels=c('patients','controls')))%>%
    select(group|task|all_of(params))%>%
    group_by(task)%>%
    dplyr::summarise(corr_alpha=cor.test(alpha_win,alpha_loss)$estimate,
                     pval_alpha=cor.test(alpha_win,alpha_loss)$p.value,
                     corr_sens=cor.test(sensitivity_win,sensitivity_loss)$estimate,
                     pval_sens=cor.test(sensitivity_win,sensitivity_loss)$p.value)
}

names(correlations)<-multiverse_list

correlations<-bind_rows(correlations,.id='analysis')

ggplot(correlations,aes(x=task,y=corr_sens))+
  geom_point(aes(colour=analysis))+
  stat_summary(geom='pointrange',fun.data='mean_se',position='jitter')+
  theme_classic()
...
Illustrative plot for effect of task on effect size estimates

```{r}
for (analysis in 1:length(multiverse_list)){
  name<-load(file=paste0(workingdir,'simulated_data/effsizes_bma_bytask_',multiverse_list[analysis]))
  name<-get(name)%>%
    pivot_longer(cols=-c(task),names_to='parameter')%>%
    separate(parameter,into=c('parameter','measure'),sep='_(?=[^_]+$)')  #regex that gets final underscore
  assign(paste0(multiverse_list[analysis]),name)
}

effectsizes_task<-bind_rows(list(map_1=map_1,map_2=map_2,vba_1=vba_1,vba_2=vba_2),.id='method')

effectsizes_task$parameter<-recode(effectsizes$parameter,'win learning rate'='win alpha','loss learning rate'='loss alpha','learning rate'='alpha',
                              'alpha_win'='win alpha','alpha_loss'='loss alpha', 'beta_win'='win temperature','beta_loss'='loss temperature', 'sensitivity_win'='win sensitivity', 'sensitivity_loss'='loss sensitivity')

effectsizes_task<-effectsizes_task%>%
  separate(parameter,c('valence','parameter'),sep=' ',fill='left')%>%
  separate(method,c('method','priors'))%>%
  pivot_wider(id_cols=-c(measure,value),names_from=measure,values_from=value)

#change valence to 'none' rather than NA
effectsizes_task$valence[is.na(effectsizes_task$valence)]<-'none'

#remove duplicates
effectsizes_task<-effectsizes_task%>%distinct()

#create interaction column
effectsizes_task$variable<-interaction(effectsizes_task$method,
                                  effectsizes_task$priors,
                                  sep='; ')

ggplot(effectsizes_task[effectsizes_task$parameter=='sensitivity',],aes(x=valence,y=estimate,fill=estimate))+
  ...
  #annotate("text", x = 1, y = 0.5*limits[2], colour = "grey", label='greater in patients',size=3)+
  #annotate("text", x = 1, y = 0.5*limits[1], colour = "grey", label='greater in controls',size=3)+
  coord_flip()
```
...
```{r}
  # parameters_long<-parameters_wide%>%
  #   pivot_longer(cols=c(3:last_col()))%>%
  #   filter(complete.cases(value))
  # 
  # ggplot(parameters_long,aes(x=value,colour=group,fill=group))+
  #   geom_histogram(size=1)+
  #   facet_grid(group~name,scales="free")+
  #   theme_minimal()+
  #   ggsave(filename=paste0(workingdir,'/figures/bma_results_',method, '.png'),width=14,height=10)
  # 
  # ggplot(parameters_long,aes(x=name,y=value,colour=group,fill=group))+
  #   geom_boxplot()+
  #   facet_wrap(.~name)+
  #   theme_minimal()+
  #   ggsave(filename=paste0(workingdir,'/figures/bma_results_',method, '_boxplots.png'),width=14,height=10)
```
