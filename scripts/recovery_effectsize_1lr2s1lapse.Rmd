...
if(newparams==1){
  ...
  lr<-generate_bounded_data(n_con=sum(paper_params$group==0),n_pat=sum(paper_params$group==1),effsize=0,0,1,tol)
  winsens<-generate_bounded_data(n_con=sum(paper_params$group==0),n_pat=sum(paper_params$group==1),effsize=0,0,10,tol)
  losssens<-generate_bounded_data(n_con=sum(paper_params$group==0),n_pat=sum(paper_params$group==1),effsize=effectsize,0,10,tol)
  lapse<-generate_bounded_data(n_con=sum(paper_params$group==0),n_pat=sum(paper_params$group==1),effsize=0,0,1,tol)
  
  
  paper_params$alpha[paper_params$group==1]<-lr$pat
  paper_params$alpha[paper_params$group==0]<-lr$con
  paper_params$winsens[paper_params$group==1]<-winsens$pat
  paper_params$winsens[paper_params$group==0]<-winsens$con
  paper_params$losssens[paper_params$group==1]<-losssens$pat
  paper_params$losssens[paper_params$group==0]<-losssens$con
  paper_params$lapse[paper_params$group==1]<-lapse$pat
  paper_params$lapse[paper_params$group==0]<-lapse$con
  
  paper_params<-paper_params%>%
    select(study,group,alpha,winsens,losssens,lapse)
...
if(newchoices==1){
  ...
  for (taskname in tasklist){
    ...
    for (participant in 1:length(paper_params$group)){
      parameters<-list(alpha=paper_params$alpha[participant],rewsens=paper_params$winsens[participant],punsens=paper_params$losssens[participant],lapse=paper_params$lapse[participant])  
      choices<- simulate_choices(parameters, task, gng)
      if (gng==0){
...
load(paste0(paramdir,'/pat_data_task1.RData'))
load(paste0(paramdir,'/con_data_task1.RData'))
...
cohen.d(get_posterior_mean(fit_1lr2s1lapse_PA,'alpha'),get_posterior_mean(fit_1lr2s1lapse_HC,'alpha'))

cohen.d(paper_params$winsens[paper_params$group==1],paper_params$winsens[paper_params$group==0])
cohen.d(parameters_wide$sensitivity_win[parameters_wide$group=='patients'],parameters_wide$sensitivity_win[parameters_wide$group=='controls'],na.rm=TRUE)
cohen.d(get_posterior_mean(fit_1lr2s1lapse_PA,'sensitivity_win'),get_posterior_mean(fit_1lr2s1lapse_HC,'sensitivity_win'))

cohen.d(paper_params$losssens[paper_params$group==1],paper_params$losssens[paper_params$group==0])
cohen.d(parameters_wide$sensitivity_loss[parameters_wide$group=='patients'],parameters_wide$sensitivity_loss[parameters_wide$group=='controls'],na.rm=TRUE)
cohen.d(get_posterior_mean(fit_1lr2s1lapse_PA,'sensitivity_loss'),get_posterior_mean(fit_1lr2s1lapse_HC,'sensitivity_loss'))


cohen.d(paper_params$lapse[paper_params$group==1],paper_params$lapse[paper_params$group==0])
cohen.d(parameters_wide$lapse[parameters_wide$group=='patients'],parameters_wide$lapse[parameters_wide$group=='controls'],na.rm=TRUE)
cohen.d(get_posterior_mean(fit_1lr2s1lapse_PA,'lapse'),get_posterior_mean(fit_1lr2s1lapse_HC,'lapse'))
