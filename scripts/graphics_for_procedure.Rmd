...
effects<-data.frame(analysis=c('map; 1','map; 2','vba; 1','vba; 2'),value=c(0.1,0.4,0.2,-0.45),lower=c(0.099,0.39,0.16,-0.451),upper=c(0.101,0.41,0.24,-0.449))

overall<-mean(effects$value)
overall.lower<-overall-0.01
overall.upper<-overall+0.01

labels<-c('Fixed-Effects Meta-Analysis',rev(unique(as.character(effects$analysis))))
...
ggplot(effects,aes(x=analysis,y=value,fill=value))+
  ...
  geom_rect(aes(xmin=0.8, xmax=1.2, ymin=overall.lower, ymax=overall.upper),alpha=0.3,fill='forestgreen')+
  ...
  annotate("text", x = 0.65, y = 0.2, colour = "grey", label='greater in patients')+
  annotate("text", x = 0.65, y = -0.2, colour = "grey", label='greater in controls')+

ggsave(paste0(workingdir,'/figures/example_forestplot.png'),width=10,height=5,scale=0.6)
