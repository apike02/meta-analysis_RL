...
transformed parameters {
  ...
  for (p in 1:nsub){
    ...
    pav_bias[p] = pav_bias_a + pav_bias_b*pav_bias_raw[p];
  }
}
...
model {
  ...
  pav_bias_a ~ normal(0,3);
  pav_bias_b ~ cauchy(0,5);
  ...
  for (p in 1:nsub){ // run the model for each subject
    ...
    for (t in 1:ntrials){
      ...
      Q_temp[1] = Q_go[stim[t]] + go_bias[p] + pav_bias[p]*value[stim[t]]; 
