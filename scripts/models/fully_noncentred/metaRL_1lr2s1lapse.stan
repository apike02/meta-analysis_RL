...
parameters {
  ...
  real alpha_a; 
  real sensitivity_win_a;
  real sensitivity_loss_a;
  real lapse_a;
  ...
  real alpha_raw[nsub]; 
  real sensitivity_win_raw[nsub];   
  real sensitivity_loss_raw[nsub];
  real lapse_raw[nsub];
}
...
transformed parameters {
  ...
  for (p in 1:nsub){
    ...
    sensitivity_win[p] = exp(sensitivity_win_a + sensitivity_win_b*sensitivity_win_raw[p]);
    sensitivity_loss[p] = exp(sensitivity_loss_a + sensitivity_loss_b*sensitivity_loss_raw[p]);
...
model {
  ...
  alpha_a ~ normal(0,3);
  alpha_b ~ cauchy(0,5);
  sensitivity_win_a ~ normal(0,3);
  sensitivity_win_b ~ cauchy(0,5);
  sensitivity_loss_a ~ normal(0,3);
  ...
  for (p in 1:nsub){ // run the model for each subject
    ...
    for (t in 1:ntrials){
      choices[t,p] ~ categorical_logit((1-lapse[p])*(Q_reward-Q_punish)+lapse[p]/2);

      // learning model
      Q_reward[choices[t,p]] = Q_reward[choices[t,p]] + alpha[p] * (sensitivity_win[p]*reward[choices[t,p],t] - Q_reward[choices[t,p]]); 
      Q_punish[choices[t,p]] = Q_punish[choices[t,p]] + alpha[p] * (sensitivity_loss[p]*punish[choices[t,p],t] - Q_punish[choices[t,p]]); 
    }
  }
}
...
generated quantities {
  ...
  
  {
    ...
    for (p in 1:nsub){ // run the model for each subject
      ...
      for (t in 1:ntrials){
        loglik[p] += categorical_logit_lpmf(choices[t,p]|(Q_reward-Q_punish)+lapse[p]/2);
