...
model {
  ...
  for (p in 1:nsub){ // run the model for each subject
    ...
    for (t in 1:ntrials){
      choices[t,p] ~ categorical_logit((Q_reward-Q_punish)*beta[p]-(perseverance[p]*choice_trace));
      // learning model
      Q_reward[choices[t,p]] = Q_reward[choices[t,p]] + alpha_win[p] * (reward[choices[t,p],t] - Q_reward[choices[t,p]]); 
      Q_punish[choices[t,p]] = Q_punish[choices[t,p]] + alpha_loss[p] * (punish[choices[t,p],t] - Q_punish[choices[t,p]]); 
...
generated quantities {
  ...
  
  {
    ...
    for (p in 1:nsub){ // run the model for each subject
      ...
      for (t in 1:ntrials){
        loglik[p] += categorical_logit_lpmf(choices[t,p]|((Q_reward-Q_punish)*beta[p]-(perseverance[p]*choice_trace)));
