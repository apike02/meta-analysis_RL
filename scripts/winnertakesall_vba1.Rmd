...
ggsave(paste0(workingdir,'/figures/bic_sums_vba_unordered_overall_sp.png'),scale=0.5,width=10,height=10,bic_sums_unordered)
...
```{r}
beta_t1<-get_posterior_mean(fit_t1,'beta')
beta_t2<-get_posterior_mean(fit_t2,'beta')
beta_t3<-get_posterior_mean(fit_t3,'beta')
beta_t4<-get_posterior_mean(fit_t4,'beta')
beta_t5<-get_posterior_mean(fit_t5,'beta')


beta_df<-cbind(beta_t1,beta_t2,beta_t3,beta_t4,beta_t5)

beta_df<-data.frame(data_details,beta_df)

beta_df<-melt(beta_df,id.vars = c(1:ncol(data_details)))
...
beta_df %>%
  group_by(pat_con)%>%
  summarise(median=round(median(`beta`),2),iqr=round(IQR(`beta`),2))


if (permute==TRUE){
  anova<-permuco::aovperm(`beta` ~ pat_con * study * task + Error(fullid/(task)), data=beta_df, method = "Rd_kheradPajouh_renaud")
} else {
...
effect_sizes_text(workingdir,'omnibus','single','vba','beta','all',beta_df)
...
source("https://gist.githubusercontent.com/benmarwick/2a1bb0133ff568cbe28d/raw/fb53bd97121f7f9ce947837ef1a4c65a73bffb3f/geom_flat_violin.R")
p2<-ggplot(beta_df,aes(x=pat_con,y=log(`beta`),
        ...
        #stat_summary(fun.data = mean_se, geom = "errorbar", width=0.1,
        #             show.legend = FALSE)+
        labs(x='Group',y='Log inverse temperature')+
        theme_classic()+
        theme(text=element_text(size=16))+
        scale_x_discrete(labels=c("0"="Control","1"="Patient"))+
        scale_fill_manual(values=c('#FC766AFF','#5B84B1FF'))+
        ...
        geom_segment(x = 2, xend = 2, 
                     y = 4, yend = 4.5,
                     colour = "black")
ggsave(paste0(workingdir,'/figures/beta_overall_sp.png'), scale=0.6)

```

Decay
```{r}
decay_t1<-get_posterior_mean(fit_t1,'decay')
decay_t2<-get_posterior_mean(fit_t2,'decay')
decay_t3<-get_posterior_mean(fit_t3,'decay')
decay_t4<-get_posterior_mean(fit_t4,'decay')
decay_t5<-get_posterior_mean(fit_t5,'decay')


decay_df<-cbind(decay_t1,decay_t2,decay_t3,decay_t4,decay_t5)

decay_df<-data.frame(data_details,decay_df)

decay_df<-melt(decay_df,id.vars = c(1:ncol(data_details)))
...
effect_sizes_text(workingdir,'omnibus','single','vba','decay','all',decay_df)
...
source("https://gist.githubusercontent.com/benmarwick/2a1bb0133ff568cbe28d/raw/fb53bd97121f7f9ce947837ef1a4c65a73bffb3f/geom_flat_violin.R")
p3<-ggplot(decay_df,aes(x=pat_con,y=`decay`,
                              group=pat_con,fill=pat_con))+
        geom_flat_violin(position = position_nudge(x = .2, y = 0),adjust = 2)+
        geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.1,
                   show.legend = FALSE, color='grey')+
        ...
        geom_segment(x = 2, xend = 2, 
                     y = 1.02, yend = 1.05,
                     colour = "black")
ggsave(paste0(workingdir,'/figures/decay_overall_sp.png'), scale=0.6)

```
Perseverance

```{r}
perseverance_t1<-get_posterior_mean(fit_t1,'perseverance')
perseverance_t2<-get_posterior_mean(fit_t2,'perseverance')
perseverance_t3<-get_posterior_mean(fit_t3,'perseverance')
perseverance_t4<-get_posterior_mean(fit_t4,'perseverance')
perseverance_t5<-get_posterior_mean(fit_t5,'perseverance')

perseverance_df<-cbind(perseverance_t1,perseverance_t2,perseverance_t3,perseverance_t4,perseverance_t5)

perseverance_df<-data.frame(data_details,perseverance_df)

perseverance_df<-melt(perseverance_df,id.vars = c(1:ncol(data_details)))
...
effect_sizes_text(workingdir,'omnibus','single','vba','perseverance','all',perseverance_df)
...
p4<-ggplot(perseverance_df,aes(x=pat_con,y=`perseverance`,group=pat_con,fill=pat_con))+
        ...
        geom_segment(x = 2, xend = 2, 
                     y = 7, yend = 8,
                     colour = "black")
ggsave(paste0(workingdir,'/figures/perseverance_overall_sp.png'), scale=0.6)
...
p1 + p2 + ylim(c(-3,8)) + p3 +ylim(c(0,1.5)) + p4 + plot_annotation(tag_levels='A') + plot_layout(ncol=2,guides='collect') 

ggsave(paste0(workingdir,'/figures/params_overall_1lr1t1d1p_vb_sp.png'), scale=0.6, width=20, height =20)

```
Get genchoices for best fitting model (using VB)
```{r}
...
for (t in 1:length(tasklist)){
  task<-tasklist[t]
  overall[[t]]<-generate_choices_fromfit(model,task,workingdir,nprior=1,method='vb')
...
accuracyplot<-ggplot(accuracy_df,aes(x=trial,y=value,colour=type))+
  ...
  ylim(c(0,1))
ggsave(filename=paste0(workingdir,'/figures/vba1_replication_accuracy.png'),accuracyplot,width=10,height=6)


densityplot<-ggplot(matching_df,aes(x=replicated,group=task,colour=task))+
  geom_density()+
  theme_classic()
ggsave(filename=paste0(workingdir,'/figures/vba1_replication.png'))
...
ggsave(filename=paste0(workingdir,'/figures/vba1_replication_rewards.png'))

accuracyplot + correlationplot + plot_annotation(tag_levels = 'A')
ggsave(filename=paste0(workingdir,'/figures/vba1_choice_replication_plot.png'),width=12,height=4)
