analyse_recoverability_map<-function(model,ntrials,task) {
  ...
  fit<-readMat(paste0(workingdir,'/generate_recover/',model,'/results_map_',task,'.mat'))$results.map[,,1][[1]]
  ...
  for (param in 1:n_param){
    ...
    ggplot(params,aes(x=original_param,y=estimated_param))+
      ...
      theme_classic()+
      geom_abline()
    ggsave(paste0(savedir,'corplot_',model,'_',task,'_',param_names[param],'_map.png'))
