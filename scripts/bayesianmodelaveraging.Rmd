  ...
  for (t in 1:length(tasklist)) {
    ...
    if (multiverse_list[analysis]=='vba_2'){
      ...
      for (i in 1:length(mnames)){
        ...
        for (param in 1:n_param){
          if(round(weights_df$weights[weights_df$model_names==model]*1000)==0) {print('weight 0')}
          sub_list[[param]]<-matrix(param_data[,param],length(param_data[,param]),round(weights_df$weights[weights_df$model_names==model]*1000)) #gets a proportion of 1000 repeats in line with the weights matrix
          if(dim(sub_list[[param]])[2]>0){
...
for (analysis in 1:length(multiverse_list)){
  ...
  nparam<-parameters_wide%>%
    summarize(across(-c(task,model,fullid,draw,study,group),funs(sum(!is.na(.)))))
  
  nparam_long[[analysis]]<-nparam%>%
    pivot_longer(cols=everything(),names_to='parameter',values_to='n')
  
  countplot<-ggplot(nparam_long[[analysis]],aes(x=parameter,y=n,fill=parameter))+
    geom_bar(stat='identity')+
...
names(nparam_long)<-multiverse_list
nparam_full<-bind_rows(nparam_long,.id='analysis')

countplot<-ggplot(nparam_full,aes(x=parameter,y=n,fill=parameter))+
  geom_bar(stat='identity')+
  labs(x='Parameter',y='Number of draws')

countplot

ggsave(countplot,filename = paste0(workingdir,'/figures/total_param_representation.png'))
...
params<-c('alpha_loss','alpha_win','alpha','beta')
...
m_beta <- metagen(estimate,
             standarderror,
             data=effectsizes[effectsizes$parameter=='beta',],
             studlab=paste(priors,method),
             comb.fixed = TRUE,
             comb.random = FALSE,
             sm="SMD")
...
```{r}
data<-list()
metalist<-list('Learning rate'=m_alpha,'Reward learning rate'=m_alpha_win,'Punishment learning rate'=m_alpha_loss,'Inverse temperature'=m_beta)
for (i in 1:length(metalist)){
  meta<-metalist[[i]]
  data[[i]]<-c(meta$TE.fixed,meta$lower.fixed,meta$upper.fixed,meta$TE,meta$lower,meta$upper)
}
names(data)<-names(metalist)

overall_meta<-
  bind_cols(data)%>%
  mutate(type=c('FE.MA estimate','FE.MA lower','FE.MA upper','1.map estimate','2.map estimate','1.vba estimate','2.vba estimate','1.map lower','2.map lower', '1.vba lower','2.vba lower','1.map upper', '2.map upper', '1.vba upper','2.vba upper'))%>%
  separate(type,sep=' ',into= c('analysis','type'))%>%
  pivot_longer(cols=-c(type,analysis))%>%
  pivot_wider(id_cols=c(analysis,name),names_from=type,values_from=value)%>%
  mutate(analysis=factor(analysis,ordered=TRUE,levels=c('FE.MA','1.map','2.map','1.vba','2.vba')))%>%
  mutate(name=factor(name,ordered=TRUE,levels=c('Learning rate','Reward learning rate','Punishment learning rate','Inverse temperature')))

limits<-c(-0.4,0.4)
ggplot(overall_meta,aes(x=analysis,y=estimate,fill=estimate))+
  ...
  theme_classic()+
  geom_point(data=overall_meta[overall_meta$analysis=='FE.MA',],fill='black',colour='black',size=4,shape=18)+
  facet_grid(.~name)+
  labs(x='Analysis')+
  theme(axis.text.y = element_text(face = c('bold',rep('plain',nrow(effectsizes)))))+
  annotate("text", x = 0.7, y = 0.5*limits[2], colour = "grey", label='greater in patients',size=2)+
  annotate("text", x = 0.7, y = 0.5*limits[1], colour = "grey", label='greater in controls',size=2)+
  coord_flip()+
  ylim(limits)

ggsave(filename='C:/Users/apike/OneDrive - University College London/metaRL/figures/horizontal_forestplot.png',
       height=8,width=40,scale=0.4)
ggsave(filename='C:/Users/apike/OneDrive - University College London/metaRL/figures/horizontal_forestplot.pdf',
       height=8,width=40,scale=0.4)
...
ggsave(filename='C:/Users/apike/OneDrive - University College London/metaRL/figures/grid_forestplot.png',
       height=30,width=30,scale=0.4)
...
quality<-read.csv(paste0(workingdir,'systematic_search/quality.csv'))
...
effectsizes_detailed<-effectsizes_detailed%>%
  ...
  inner_join(quality%>%select(Authors.and.year,Total,Year),by=c('study'='Authors.and.year'))%>%
  mutate(quality=(Total-mean(Total)))%>%
...
update.meta(m_alpha_loss, 
            byvar = disorder, 
            tau.common = FALSE)
update.meta(m_invtemperature, 
            byvar = disorder, 
            tau.common = FALSE)


```
Meta regression on study quality and year

```{r}
metareg(m_alpha,~quality)
metareg(m_alpha_win,~quality)
metareg(m_alpha_loss,~quality)
metareg(m_invtemperature,~quality)


metareg(m_alpha,~year)
metareg(m_alpha_win,~year)
metareg(m_alpha_loss,~year)
metareg(m_invtemperature,~year)
