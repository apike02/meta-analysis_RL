analyse_recoverability<-function(model,ntrials,task) {
  workingdir='C:/Users/apike/OneDrive - University College London/metaRL/'
  source(paste0(workingdir,'bic.R'))
  source(paste0(workingdir,'get_previous_character.R'))
  library('reshape2')
  library('plyr')
  library('loo')
  ...
  temp<-load(paste0(workingdir,'/generated_params')) #load original parameters
  ...
  for (param in 1:n_param){
    ...
    #estimated_param<-rstan::extract(eval(parse(text=mname)),param_names[param])
    #estimated_param<-colMeans(estimated_param[[1]])
    assign('output',eval(parse(text=mname)))
    estimated_param<-output$summary(param_names[param])$mean
    ...
    ggplot(params,aes(x=original_param,y=estimated_param))+
      ...
      theme_classic()+
      geom_abline()
    ggsave(paste0(savedir,'corplot_',model,'_',task,'_',param_names[param],'_stan.png'))
