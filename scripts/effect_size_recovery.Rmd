---
title: "Test effect size recovery - generate parameters"
output: html_notebook
---

This notebook generates the parameters to use if we assume a known effect size  (see https://julianquandt.com/post/power-analysis-by-data-simulation-in-r-part-ii/)
```{r}
rm(list=ls())
...
dirname<-paste0('effectsize_',effectsize)
dir.create(paste0(workingdir,'generate_recover/',dirname))

savedir<-paste0(workingdir,'/generate_recover/',dirname,'/')
...
load(file=paste0(workingdir,'/raw_parameters/Chase_params'))

alphawin_sim<-generate_bounded_data(n_con = ncon_Chase, n_pat=npat_Chase, effsize=effectsize, lb=0, ub=1, tol)
alphaloss_sim<-generate_bounded_data(n_con = ncon_Chase, n_pat=npat_Chase, effsize=effectsize, lb=0, ub=1, tol)
tau_sim<-generate_bounded_data(n_con = ncon_Chase, n_pat=npat_Chase, effsize=effectsize, lb=0, ub=Inf, tol)

Chase_params_simulated<-Chase_params%>%
  mutate(alphawin=ifelse(group==1,alphawin_sim$pat,alphawin_sim$con))%>%
  mutate(alphaloss=ifelse(group==1,alphaloss_sim$pat,alphaloss_sim$con))%>%
  mutate(tau=ifelse(group==1,tau_sim$pat,tau_sim$con))

#nb: 6 controls and 5 patients quit task or did not reach criterion

#ignore the test phase as not learning but PIT

save(Chase_params_simulated,file=paste0(savedir,'Chase_params_simulated'))
...
load(file=paste0(workingdir,'/raw_parameters/Kumar_params'))

alphawin_sim<-generate_bounded_data(n_con = ncon_Kumar, n_pat=npat_Kumar, effsize=effectsize, lb=0, ub=1, tol)
alphaloss_sim<-generate_bounded_data(n_con = ncon_Kumar, n_pat=npat_Kumar, effsize=effectsize, lb=0, ub=1, tol)
tauwin_sim<-generate_bounded_data(n_con = ncon_Kumar, n_pat=npat_Kumar, effsize=effectsize, lb=0, ub=Inf, tol)
tauloss_sim<-generate_bounded_data(n_con = ncon_Kumar, n_pat=npat_Kumar, effsize=effectsize, lb=0, ub=Inf, tol)

Kumar_params_simulated<-Kumar_params%>%
  mutate(alphawin=ifelse(group==1,alphawin_sim$pat,alphawin_sim$con))%>%
  mutate(alphaloss=ifelse(group==1,alphaloss_sim$pat,alphaloss_sim$con))%>%
  mutate(tauwin=ifelse(group==1,tauwin_sim$pat,tauwin_sim$con))%>%
  mutate(tauloss=ifelse(group==1,tauloss_sim$pat,tauloss_sim$con))


save(Kumar_params_simulated,file=paste0(savedir,'Kumar_params_simulated'))
```
...
load(file=paste0(workingdir,'/raw_parameters/Moutoussis_params'))

alpha_sim<-generate_bounded_data(n_con = ncon_Moutoussis, n_pat=npat_Moutoussis, effsize=effectsize, lb=0, ub=1, tol)
winsens_sim<-generate_bounded_data(n_con = ncon_Moutoussis, n_pat=npat_Moutoussis, effsize=effectsize, lb=0, ub=Inf, tol)
losssens_sim<-generate_bounded_data(n_con = ncon_Moutoussis, n_pat=npat_Moutoussis, effsize=effectsize, lb=0, ub=Inf, tol)
pav_sim<-generate_bounded_data(n_con = ncon_Moutoussis, n_pat=npat_Moutoussis, effsize=effectsize, lb=-1, ub=1, tol)
go_sim<-generate_bounded_data(n_con = ncon_Moutoussis, n_pat=npat_Moutoussis, effsize=effectsize, lb=-1, ub=1, tol)
lapse_sim<-generate_bounded_data(n_con = ncon_Moutoussis, n_pat=npat_Moutoussis, effsize=effectsize, lb=0, ub=1, tol)

Moutoussis_params_simulated<-Moutoussis_params%>%
  mutate(alpha=ifelse(group==1,alpha_sim$pat,alpha_sim$con))%>%
  mutate(winsens=ifelse(group==1,winsens_sim$pa,winsens_sim$con))%>%
  mutate(losssens=ifelse(group==1,losssens_sim$pat,losssens_sim$con))%>%
  mutate(pav=ifelse(group==1,pav_sim$pat,pav_sim$con))%>%
  mutate(go=ifelse(group==1,go_sim$pat,go_sim$con))%>%
  mutate(lapse=ifelse(group==1,lapse_sim$pat,lapse_sim$con))

save(Moutoussis_params_simulated,file=paste0(savedir,'Moutoussis_params_simulated'))
...
load(file=paste0(workingdir,'/raw_parameters/Brown_params'))

alphawin_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=0, ub=1, tol)
alphaloss_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=0, ub=1, tol)
rewsens_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=0, ub=Inf, tol)
losssens_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=0, ub=Inf, tol)
pav_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=-1, ub=1, tol)
go_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=-1, ub=1, tol)
lapse_sim<-generate_bounded_data(n_con = ncon_Brown, n_pat=npat_Brown, effsize=effectsize, lb=0, ub=1, tol)

Brown_params_simulated<-Brown_params%>%
  mutate(alphawin=ifelse(group==1,alphawin_sim$pat,alphawin_sim$con))%>%
  mutate(alphaloss=ifelse(group==1,alphaloss_sim$pat,alphaloss_sim$con))%>%
  mutate(rewsens=ifelse(group==1,rewsens_sim$pa,rewsens_sim$con))%>%
  mutate(losssens=ifelse(group==1,losssens_sim$pat,losssens_sim$con))%>%
  mutate(pav=ifelse(group==1,pav_sim$pat,pav_sim$con))%>%
  mutate(go=ifelse(group==1,go_sim$pat,go_sim$con))%>%
  mutate(lapse=ifelse(group==1,lapse_sim$pat,lapse_sim$con))
