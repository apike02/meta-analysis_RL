function likelihood = fit_2lr1t1d_gng(params,fullid,simdata,tol,prior)
...
for t=1:trials
    if stim(t)==1
        ...
        Q_go = qa_go - max([qa_go qa_nogo])-choice_trace_go(1);
        Q_nogo = qa_nogo - max([qa_go qa_nogo])-choice_trace_nogo(1);
    ...
    elseif stim(t)==2
        ...
        Q_go = qb_go - max([qb_go qb_nogo])-choice_trace_go(2);
        Q_nogo = qb_nogo - max([qb_go qb_nogo])-choice_trace_nogo(2);
        prob(1)=exp(Q_go)./(exp(Q_go)+exp(Q_nogo));
        prob(2)=exp(Q_nogo)./(exp(Q_nogo)+exp(Q_go));
           if choices(t)==1
     ...
     elseif stim(t)==3
        ...
        Q_go = qc_go - max([qc_go qc_nogo])-choice_trace_go(3);
        Q_nogo = qc_nogo - max([qc_go qc_nogo])-choice_trace_nogo(3);
        prob(1)=exp(Q_go)./(exp(Q_go)+exp(Q_nogo));
        prob(2)=exp(Q_nogo)./(exp(Q_nogo)+exp(Q_go));
           if choices(t)==1
     ...
     elseif stim(t)==4
        ...
        Q_go = qd_go - max([qd_go qd_nogo])-choice_trace_go(4);
        Q_nogo = qd_nogo - max([qd_go qd_nogo])-choice_trace_nogo(4);
        prob(1)=exp(Q_go)./(exp(Q_go)+exp(Q_nogo));
        prob(2)=exp(Q_nogo)./(exp(Q_nogo)+exp(Q_go));
           if choices(t)==1
