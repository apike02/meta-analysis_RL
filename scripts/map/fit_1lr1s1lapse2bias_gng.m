function likelihood = fit_1lr1s1lapse2bias_gng(params,fullid,simdata,tol,prior)

%1 action bias, 1 approach/avoid(Pavlovian) bias
...
for t=1:trials
    ...
    elseif stim(t)==2
        go_weight = qb_go + act_bias + app_bias*valueB;
        nogo_weight = qb_nogo;
        g_w=go_weight-max([go_weight nogo_weight]);
        ng_w=nogo_weight-max([go_weight nogo_weight]);
        prob(1)=(1-lapse).*exp(g_w)./(exp(g_w)+exp(ng_w))+lapse/2;
     ...
     elseif stim(t)==4
        go_weight = qd_go + act_bias + app_bias*valueD;
        nogo_weight = qd_nogo;
        g_w=go_weight-max([go_weight nogo_weight]);
        ng_w=nogo_weight-max([go_weight nogo_weight]);
        prob(1)=(1-lapse).*exp(g_w)./(exp(g_w)+exp(ng_w))+lapse/2;
