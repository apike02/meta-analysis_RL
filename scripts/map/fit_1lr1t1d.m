function likelihood = fit_1lr1t1d(params,fullid,simdata,tol,prior)
...
for t=1:trials
    QA=qa-max([qa qb]);
    QB=qb-max([qa qb]);
    prob(1)=exp(QA.*beta-choice_trace(1))./(exp(QA.*beta-choice_trace(1))+exp(QB.*beta-choice_trace(2)));
    prob(2)=exp(QB.*beta-choice_trace(2))./(exp(QB.*beta-choice_trace(2))+exp(QA.*beta-choice_trace(1)));
