function likelihood = fit_1lr1s1lapse3bias_gng(params,fullid,simdata,tol,prior)

%1 action bias, 1 approach bias, and 1 avoid bias
...
for t=1:trials
    if stim(t)==1
        go_weight = qa_go + act_bias + app_bias*valueA;
        nogo_weight = qa_nogo;
        ...
        prob(2)=(1-lapse).*exp(ng_w)./(exp(ng_w)+exp(g_w))+lapse/2;
           if choices(t)==1
            ...
            valueA=valueA + alpha*(sensitivity*go_outcome(t) - valueA);
           else 
            qa_nogo=qa_nogo+alpha*(sensitivity*nogo_outcome(t) - qa_nogo);
            valueA=valueA + alpha*(sensitivity*nogo_outcome(t) - valueA);
           end
    elseif stim(t)==2
        go_weight = qb_go + act_bias + av_bias*valueB;
        nogo_weight = qb_nogo;
        g_w=go_weight-max([go_weight nogo_weight]);
        ng_w=nogo_weight-max([go_weight nogo_weight]);
        prob(1)=(1-lapse).*exp(g_w)./(exp(g_w)+exp(ng_w))+lapse/2;
        prob(2)=(1-lapse).*exp(ng_w)./(exp(ng_w)+exp(g_w))+lapse/2;
           if choices(t)==1
            ...
            valueB=valueB + alpha*(sensitivity*go_outcome(t) - valueB);
           else 
            qb_nogo=qb_nogo+alpha*(sensitivity*nogo_outcome(t) - qb_nogo);
            valueB=valueB + alpha*(sensitivity*nogo_outcome(t) - valueB);
           end
     elseif stim(t)==3
        go_weight = qc_go + act_bias + app_bias*valueC;
        nogo_weight = qc_nogo;
        g_w=go_weight-max([go_weight nogo_weight]);
        ng_w=nogo_weight-max([go_weight nogo_weight]);
        prob(1)=(1-lapse).*exp(g_w)./(exp(g_w)+exp(ng_w))+lapse/2;
        prob(2)=(1-lapse).*exp(ng_w)./(exp(ng_w)+exp(g_w))+lapse/2;
           if choices(t)==1
            ...
            valueC=valueC + alpha*(sensitivity*go_outcome(t) - valueC);
           else 
            qc_nogo=qc_nogo+alpha*(sensitivity*nogo_outcome(t) - qc_nogo);
            valueC=valueC + alpha*(sensitivity*nogo_outcome(t) - valueC);
           end
     elseif stim(t)==4
        go_weight = qd_go + act_bias + av_bias*valueD;
        nogo_weight = qd_nogo;
        g_w=go_weight-max([go_weight nogo_weight]);
        ng_w=nogo_weight-max([go_weight nogo_weight]);
        prob(1)=(1-lapse).*exp(g_w)./(exp(g_w)+exp(ng_w))+lapse/2;
        prob(2)=(1-lapse).*exp(ng_w)./(exp(ng_w)+exp(g_w))+lapse/2;
           if choices(t)==1
            ...
            valueD=valueD + alpha*(sensitivity*go_outcome(t) - valueD);
           else 
...
if nargin<4
    ...
    if group==1
        ...
        norm_lpmf(app_bias,prior{5,1}(1),prior{5,1}(2),tol)+...
        norm_lpmf(av_bias,prior{6,1}(1),prior{6,1}(2),tol);
    else 
        %1 is param 1, 2 is group 2 (con), 1./2 is first or second prior param
        ...
        norm_lpmf(app_bias,prior{5,2}(1),prior{5,2}(2),tol)+...
        norm_lpmf(av_bias,prior{6,2}(1),prior{6,2}(2),tol);
    end
