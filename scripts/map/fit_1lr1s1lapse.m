function likelihood = fit_1lr1s1lapse(params,fullid,simdata,tol,prior)
...
for t=1:trials
    QA=qa-max([qa qb]);
    QB=qb-max([qa qb]);
    prob(1)=(1-lapse).*exp(QA)./(exp(QA)+exp(QB))+lapse./2;
    prob(2)=(1-lapse).*exp(QB)./(exp(QB)+exp(QA))+lapse./2;
