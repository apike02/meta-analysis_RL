function likelihood = fit_1lr1t_gng(params,fullid,simdata,tol,prior)
...
for t=1:trials
    if stim(t)==1
        ...
        prob(1)=exp(Q_go.*beta)./(exp(Q_go.*beta)+exp(Q_nogo.*beta));
        prob(2)=exp(Q_nogo.*beta)./(exp(Q_nogo.*beta)+exp(Q_go.*beta));
           if choices(t)==1
    ...
    elseif stim(t)==2
        ...
        prob(2)=exp(Q_nogo.*beta)./(exp(Q_nogo.*beta)+exp(Q_go.*beta));
           if choices(t)==1
            qb_go=qb_go+alpha*(go_outcome(t) - qb_go); 
           else 
     ...
     elseif stim(t)==3
        ...
        prob(2)=exp(Q_nogo.*beta)./(exp(Q_nogo.*beta)+exp(Q_go.*beta));
           if choices(t)==1
            qc_go=qc_go+alpha*(go_outcome(t) - qc_go); 
           else 
     ...
     elseif stim(t)==4
        ...
        prob(2)=exp(Q_nogo.*beta)./(exp(Q_nogo.*beta)+exp(Q_go.*beta));
           if choices(t)==1
            qd_go=qd_go+alpha*(go_outcome(t) - qd_go); 
           else 
