function likelihood = fit_2lr2t_gng(params,fullid,simdata,tol,prior)
...
for t=1:trials
    if stim(t)==1
        qa_go = (qa_rew_go.*beta_rew + qa_pun_go.*beta_pun);
        qa_nogo = (qa_rew_nogo.*beta_rew + qa_pun_nogo.*beta_pun);
    ...
    elseif stim(t)==2
        qb_go = (qb_rew_go.*beta_rew + qb_pun_go.*beta_pun);
        qb_nogo = (qb_rew_nogo.*beta_rew + qb_pun_nogo.*beta_pun);
     ...
     elseif stim(t)==3
        qc_go = (qc_rew_go.*beta_rew + qc_pun_go.*beta_pun);
        qc_nogo = (qc_rew_nogo.*beta_rew + qc_pun_nogo.*beta_pun);
     ...
     elseif stim(t)==4
        qd_go = (qd_rew_go.*beta_rew + qd_pun_go.*beta_pun);
        qd_nogo = (qd_rew_nogo.*beta_rew + qd_pun_nogo.*beta_pun);
        prob(1)=exp(qd_go)/(exp(qd_go)+exp(qd_nogo));
        prob(2)=exp(qd_nogo)/(exp(qd_nogo)+exp(qd_go));
           if choices(t)==1
...
if nargin<4
    ...
    if group==1
        ...
        gamma_lpmf(beta_rew,prior{3,1}(1),prior{3,1}(2),tol)+...
        gamma_lpmf(beta_pun,prior{4,1}(1),prior{4,1}(2),tol);
    else 
        %1 is param 1, 2 is group 2 (con), 1./2 is first or second prior param
        log_prior=beta_lpmf(alpha_rew,prior{1,2}(1),prior{1,2}(2),tol)+...
        beta_lpmf(alpha_pun,prior{2,2}(1),prior{2,2}(2),tol)+...
        gamma_lpmf(beta_rew,prior{3,2}(1),prior{3,2}(2),tol)+...
        gamma_lpmf(beta_pun,prior{4,2}(1),prior{4,2}(2),tol);
    end
